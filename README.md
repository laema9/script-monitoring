...
\- \*\*`hostname\_var`\*\* — nom d'hôte de la machine
...
!\[services](screenshots/etats\_services.png)
...
\- \*\*`journald\_error.txt`\*\* — uniquement les lignes contenant `ERROR` ou `CRITICAL`, filtrées via `grep -E "ERROR|CRITICAL"`



\---

!\[logs](screenshots/lecture\_logs.png)
...
\- \*\*`top\_cpu.txt`\*\* — les 10 processus les plus consommateurs de CPU à l'instant de l'exécution
...
!\[cpu](screenshots/observation\_processus.png)
...
\*\*`app\_redacted.txt`\*\* est une copie du log dans laquelle les valeurs sensibles sont masquées :



```bash

sed -E 's/(secret|password|token|key)=\[a-zA-Z0-9\_-]+/\\1=REDACTED/gI'
...
!\[app](screenshots/analyse\_app\_log.png)
...
!\[metrics](screenshots/analyse\_metrics.png)
...
\- \*\*`tree\_bak.txt`\*\* — liste de tous les fichiers portant l'extension `.bak`

\- \*\*`tree\_secret.txt`\*\* — liste des fichiers contenant la chaîne `SECRET=` (recherche via `xargs grep -l`)



\---

!\[tree](screenshots/fichier\_temp.png)
...
!\[controle](screenshots/controle\_des\_fichiers.png)
...
| `journald\_error.txt` | Logs filtrés sur `ERROR` et `CRITICAL` |

| `top\_cpu.txt` | Top 10 des processus par consommation CPU |

| `hogs.txt` | Processus `cpu-hog` détectés |

| `app\_summary.txt` | Statistiques et top 5 hôtes du log applicatif |

| `app\_redacted.txt` | Log applicatif avec secrets masqués |

| `metrics\_summary.txt` | Statistiques CSV et lignes CPU ≥ 95 % |

| `tree\_bak.txt` | Fichiers `.bak` dans le répertoire temporaire |

| `tree\_secret.txt` | Fichiers contenant `SECRET=` |
...
then nouveauservice\_etat='Ok'

else nouveauservice\_etat='Nok'
...
La redaction des secrets dans `app\_redacted.txt` est gérée par cette expression `sed` :
...
sed -E 's/(secret|password|token|key|apikey)=\[a-zA-Z0-9\_-]+/\\1=REDACTED/gI'
